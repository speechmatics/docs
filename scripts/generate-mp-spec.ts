...
const spec = yaml.parse(readFileSync("spec/mp.yaml", "utf8"));
...
spec.basePath = "https://mp.api.speechmatics.com/v1";
    ...
    if (method in data) {
      ...
      try {
        sampleFiles = readdirSync(
          `${projectRoot}/spec/code-samples/mp-api${path}`,
        ).filter(
      ...
      data[method]["x-codeSamples"] = sampleFiles.map((file) => ({
        ...
        source: readFileSync(
          `${projectRoot}/spec/code-samples/mp-api${path}/${file}`,
          "utf8",
        ),
      }));
    }
...
writeFileSync("static/mp-api.yaml", yaml.stringify(spec));
