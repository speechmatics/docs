...
### Basic parameters
`language` (str, default: "en") Language code for transcription (e.g., "en", "es", "fr"). See [supported languages](/speech-to-text/languages).

`operating_point` (OperatingPoint, default: ENHANCED) Balance accuracy vs latency. Options: STANDARD or ENHANCED.

`domain` (str, default: None) Domain-specific model (e.g., "finance", "medical"). See [supported languages and domains](/speech-to-text/languages).

`output_locale` (str, default: None) Output locale for formatting (e.g., "en-GB", "en-US"). See [supported languages and locales](/speech-to-text/languages).
