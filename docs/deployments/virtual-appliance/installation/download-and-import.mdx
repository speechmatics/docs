...
GPU transcription requires PCIe passthrough of a [suitable graphics card](/deployments/container/gpu-speech-to-text#system-requirements) from the host to the virtual appliance. A prerequisite of this is to use a suitable virtual machine type along with a UEFI bios, like so:
