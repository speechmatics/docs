...
Melia 1 is a multilingual, GPU-only model. For an overview, see [Models](/speech-to-text/models#melia-1).
Real-time mode is currently in preview, see preview docs
...
The transcriber is a single unified image for both batch and real-time, so the SM_ASR_MODE environment variable must also be set, to batch or rt.
