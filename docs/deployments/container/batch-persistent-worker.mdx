...
| Code | Reason |
|---|---|
| `404` | Log for job ID `{job_id}` not found |
| `404` | No log file found for job `{job_id}`. Verbosity isn't enough to produce debug logs. Add `-v` to the container arguments or set the environment variable `DEBUG=true` for higher verbosity. |
