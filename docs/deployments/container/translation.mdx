...
_Note: System requirements for the Translation inference server are the same as for the [GPU Inference Container](/deployments/container/gpu-speech-to-text-container) for transcription, except for RAM and CPU requirements which are lower. The two servers cannot use the same GPU. _
...
Batch and Real-Time (RT) transcribers handle sending requests to the Translation Inference Server. To run a transcription job with Translation, follow the [instructions for running the CPU Container](/deployments/container/cpu-speech-to-text-container) and additionally:
...
See the [instructions](/deployments/container/gpu-speech-to-text-container#deploying-on-kubernetes) for deploying the Transcription GPU Inference Server.
