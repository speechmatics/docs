...
[Speaker Identification](/speech-to-text/features/speaker-identification) requires the Batch or Real-Time Transcriber to access a set of cryptographically strong secrets. These secrets are used internally to generate and verify speaker identifiers. This section explains how to prepare the secret files and how to mount them into the Container in both Batch and Real-Time deployments.
...
docker run --rm -i
 ...
 -e SM_SPEAKER_ID_SECRETS_DIR=/speaker_id_secrets 
 [other arguments]
```

The Transcriber will automatically load all secret files in the specified directory when it starts.

### Using Speaker Identification on Kubernetes

When deploying on Kubernetes, the secret directory can be mounted using any suitable volume type, such as hostPath, secret, or emptyDir (if the secret is injected by an initContainer or sidecar). The mounted path must be made available to the Container and referenced via the `SM_SPEAKER_ID_SECRETS_DIR` environment variable. Below is an example fragment:

```yaml
volumeMounts:
  - name: speaker-id-secrets
    mountPath: /speaker_id_secrets
    readOnly: true

env:
  - name: SM_SPEAKER_ID_SECRETS_DIR
    value: /speaker_id_secrets

volumes:
  - name: speaker-id-secrets
    hostPath:
      path: /path/on/node/with/speaker-id-secrets
      type: Directory
```

This setup allows the Transcriber to access the speaker identification secrets securely, whether running in a container environment with strict filesystem controls or within a managed Kubernetes deployment.