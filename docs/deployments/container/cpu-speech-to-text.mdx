...
Batch persistent workers (known as http batch workers), are batch multi session capable persistent workers. They work utilizing an http server, which is able to 
...
- All data is transitory, once a container completes its transcription it removes all record of the operation, no data is persistent
