...
Once the GPU Server is running, follow the [Instructions for Linking a CPU Container](/deployments/container/cpu-speech-to-text-container#linking-to-a-gpu-inference-container).
...
services:
  triton:
    image: speechmaticspublic.azurecr.io/sm-gpu-inference-server-en:${smVariables.latestContainerVersion}
    deploy:
      resources:
        reservations:
          devices:
