...
To use our latest and most accurate models, please refer to the [Transcription GPU Container](/deployments/container/gpu-speech-to-text-container) deployment.
...
### Running in multi-session mode (Real-time)
...
Most memory is shared between connections, but you should expect CPU usage to scale with the number of active sessions.

When the transcription container is linked to a GPU inference server, more memory can be shared because the larger models are handled by the remote GPU. 
...
For more details see [GPU Inference Container](/deployments/container/gpu-speech-to-text-container)
