...
keywords: [speechmatics, container, gpu, GPU, NVIDIA, CUDA, SaaS, on-prem]
---
...
- [A license file or a license token](/deployments/container/licensing)
  - There is no specific license for the GPU Inference Container, it will run using an existing Speechmatics license for the Realtime or Batch
    Container
...
:::note
This section does not apply to `melia-1`. It is provided as a dedicated image and is loaded by default, so you do not set `SM_MODEL` or `SM_OPERATING_POINT`.
:::
...
When inference is outsourced to a GPU server, alternative GPU-specific models are used, so you should not expect to see identical results compared to CPU-based inference. For convenience, the GPU models are also designated as 'standard', 'enhanced' and 'melia-1'.
