---
description: 'Learn about automatic usage reporting for on-prem deployments'
---
...
The Realtime transcriber will report one `SESSION_ENDED` event at the end of each session. During a session, the Realtime transcriber also sends a `SESSION_STATUS` event roughly every five minutes. `SESSION_STATUS` carries the same duration fields as `SESSION_ENDED` (for example `session_speech_duration` and `session_transcribed_duration`), but reports them cumulatively for the session so far, rather than as a delta since the last event.

Example `SESSION_STATUS` payload:

```json
{
  "event": "SESSION_STATUS",
  "session_speech_duration": 118.42,
  "session_transcribed_duration": 300.0,
...
If a session is terminated abnormally rather than closed in the usual way, the transcriber instead emits a `SESSION_FORCE_ENDED` event in place of `SESSION_ENDED`, carrying the same final cumulative duration fields.
...
- Realtime Container `10.1.0` onwards
