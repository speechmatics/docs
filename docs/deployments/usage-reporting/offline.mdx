...
Learn more about [what data we record](/deployments/usage-reporting#what-data-do-we-record).
:::
...
For details on how to export usage data for a given on-prem solution see here [Container](../containers/usage.mdx#exporting-usage-data) and [Appliance](../virtual-appliance/administration/usage.mdx#exporting-usage-data) for more details.
...
- Before running any jobs, the user sets the usage mode to `offline` see [above](#setting-the-usage-mode)
...
Currently, Speechmatics requires data to be sent via email to [billing-reporting@speechmatics.com](mailto:billing-reporting@speechmatics.com).
...
The complete API reference for extracting usage data can be found in the [API Reference](../../../static/api/jobs-spec-final.yaml) section.
...
Once the data has been exported, it must be emailed as attachment(s) to [mailto:billing-reporting@speechmatics.com](mailto:billing-reporting@speechmatics.com). Any exported files should remain compressed when sending to Speechmatics.
