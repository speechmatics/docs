---
pagination_prev: null
pagination_next: null
description: Take your first steps with the Speechmatics API.
---
import { LinkCard } from "@site/src/theme/LinkCard";
import DevIcon from '@site/src/components/devicon';
import { Flex, Grid } from "@radix-ui/themes";
import { ChevronsRightIcon, FileAudio, BotMessageSquare, BookOpen, Braces, Speech, GraduationCap } from "lucide-react";
import Head from '@docusaurus/Head';

<Head>
  <link rel="stylesheet" href="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/devicon.min.css" />
</Head>

# Quickstart

:::tip
The easiest way to try our products is via the [web portal](https://portal.speechmatics.com/).
:::

## Explore our developer guides

<Flex direction="column" gap="3">
  <LinkCard
    title="Realtime transcription"
    description="Instantly convert streaming audio to text with the Realtime SaaS WebSocket API"
    icon={<ChevronsRightIcon/>}
    href="/speech-to-text/realtime/quickstart"
  />
  <LinkCard
    title="Transcribe a file"
    description="Use Batch processing to accurately turn your audio files into text with Batch API"
    icon={<FileAudio/>}
    href="/speech-to-text/batch/quickstart"
  />
  <LinkCard
    title="Build a voice agent"
    description="Build conversational AI agents with ease using our integrations"
    icon={<BotMessageSquare/>}
    href="/speech-to-text/agent-stt"
  />
  <LinkCard
    title="Generate speech from text"
    description="Use the TTS API to convert text to speech"
    icon={<Speech/>}
    href="/text-to-speech/quickstart"
  />
</Flex>

## Quicklinks

<Grid columns={{initial: "1", md: "2"}} gap="3">
  <LinkCard
    title="API reference"
    description="Reference documentation for all our APIs"
    icon={<BookOpen/>}
    href="/api-ref"
  />
  <LinkCard
    title="JavaScript SDK"
    description="Realtime and Batch transcription from JavaScript"
    icon={<Braces/>}
    href="https://github.com/speechmatics/speechmatics-js-sdk"
  />
  <LinkCard
    title="Python SDK"
    description="Agent STT, Realtime, Batch and TTS from Python"
    icon={<DevIcon name="python" />}
    href="https://github.com/speechmatics/speechmatics-python-sdk"
  />
  <LinkCard
    title="Speechmatics Academy"
    description="Working examples, integrations, and templates for the Speechmatics SDKs"
    icon={<GraduationCap/>}
    href="https://github.com/speechmatics/speechmatics-academy"
  />
</Grid>