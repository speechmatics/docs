...
The Batch and Realtime endpoints below are active and highly available. You can use multiple regions to balance requests or provide a failover in the event of disruption to one region. Agent STT uses a separate preview endpoint, listed in its own tab.
