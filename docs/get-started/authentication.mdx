...
For server-side calls, your API key must be provided in the header of the [WebSocket connection request](/api-ref/agent-stt-websocket#handshake-responses) to the `/v2/agent` path. For example:
...
For client-side calls, we recommend using [temporary keys](#temporary-keys) (JWTs) to authenticate your requests. Agent STT uses Realtime (`type=rt`) temporary keys. These can be passed as a query param to the [WebSocket connection request](/api-ref/agent-stt-websocket#handshake-responses) URL. For example:

```bash
wss://global.rt.speechmatics.com/v2/agent?jwt=$TEMP_KEY
```
...
| AU1 (Australia)  | **au.rt.speechmatics.com**     |
...
:::warning
`global.rt.speechmatics.com` may route a connection to any region. If you have data residency or compliance requirements, use a regional endpoint (`eu.rt.speechmatics.com`, `us.rt.speechmatics.com`, or `au.rt.speechmatics.com`) instead.
:::
  </TabItem>
  <TabItem value="agent-stt" label="Agent STT">


Speechmatics Agent STT supports the following endpoints for production use:
...
Connect to the `/v2/agent` path, for example `wss://eu.rt.speechmatics.com/v2/agent`. See the [Agent STT Reference](/api-ref/agent-stt-websocket).
...
Agent STT uses Realtime temporary keys. Request a key with `type=rt` and use it on the `/v2/agent` path — there is no separate Agent STT key type.

Temporary keys allow the end-user to start a WebSocket connection with Speechmatics directly, without exposing your long-lived API key. This reduces the latency of transcription, as well as development effort.

An Agent STT temporary key is not tied to a region. The same key works with any Agent STT endpoint, including `global.rt.speechmatics.com`.

The example request below will create a temporary key which can be used to start any number of Agent STT sessions for 60 seconds:
...
:::info
Note that when starting an Agent STT session in the browser, you must provide the temporary key as a part of a query parameter. See our documentation on [browser based transcription](/api-ref/agent-stt-websocket#browser-based-transcription).
:::
</TabItem>
</Tabs>
...
|------------------------|----------|-----------------------------------------------------------
| `type`                 | No       | String: `batch` (default), `rt`. Whether the token should be able to use the Batch API, or the Realtime and Agent STT APIs.                                                                                                                                                                                                                                                                                                     |
