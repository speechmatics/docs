...
Build a real-time voice agent with Speechmatics and LiveKit in minutes.

[LiveKit Agents](https://docs.livekit.io/agents/) is a framework for building voice AI applications using WebRTC. Plug Speechmatics STT and TTS into your voice agent pipeline. The STT plugin uses [agent STT](/speech-to-text/agent-stt), our turn-based transcription for voice agents, powered by Linden 1.

:::note

Linden 1 requires LiveKit Agents v1.8.2 or later.
...
```bash
uv init
uv add "livekit-agents[speechmatics,openai,silero]~=1.8.2" python-dotenv
```
...
Both plugins run on their defaults here. See [Speech to text](/integrations-and-sdks/livekit/stt) to configure diarization, turn detection and language, and [Text to speech](/integrations-and-sdks/livekit/tts) to choose a voice.
