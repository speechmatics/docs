...
description: Build a local voice bot with Speechmatics Speech to Text and Text to Speech using Pipecat.
---
...
[Pipecat](https://docs.pipecat.ai/) is a framework for building real-time voice bots using a pipeline architecture. In this quickstart, you'll run a local WebRTC server and connect to your bot from your browser. The STT service uses [agent STT](/speech-to-text/agent-stt), our turn-based transcription for voice agents, powered by Linden 1.

:::note

Linden 1 requires Pipecat 1.11.0 or later.

:::

## Features

- **Real-time transcription** — low-latency Speech to Text (STT) as users speak
...
pipecat-ai[local-smart-turn,silero,speechmatics,webrtc,openai,runner]~=1.11.0
