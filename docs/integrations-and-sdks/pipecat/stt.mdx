...
| `additional_vocab` | array | `[]` | `AdditionalVocabEntry` objects forming a [custom dictionary](/speech-to-text/features/custom-dictionary) |
...
Include the format in your bot's system prompt so the language model reads the labels as speaker attribution rather than as part of what was said. Two settings help when speakers are hard to separate: raise `speaker_sensitivity` to split similar-sounding voices more aggressively, and set `prefer_current_speaker` to reduce switching mid-turn. The template accepts the following attributes.
