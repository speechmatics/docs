...
:::info
The global endpoint automatically routes each connection to the nearest region. You can also pin to a specific region for data residency, for example `wss://eu.rt.speechmatics.com/v2/agent`. See [Supported endpoints](/get-started/authentication#supported-endpoints).
:::
...
The remaining received messages manage the session rather than the conversation: `RecognitionStarted`, `AudioAdded`, `SpeakersResult`, `EndOfTranscript`, `Info`, `Warning` and `Error`. See [Received messages](#received-messages).

:::warning
#### Browser based transcription

When starting an Agent STT session **in the browser**, [temporary keys](/get-started/authentication#temporary-keys) should be used to avoid exposing your long-lived API key. Agent STT uses Realtime (`type=rt`) temporary keys.
