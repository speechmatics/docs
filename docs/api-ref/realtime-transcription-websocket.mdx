...
sequenceDiagram
  participant Client
  participant API
  Client->>API: Handshake request - GET /v2/<language-code> with Auth Token
...
When starting a Real-Time transcription session **in the browser**, [temporary keys](/get-started/authentication#temporary-keys) should be used to avoid exposing your long-lived API key.
