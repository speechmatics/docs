...
|---------|--------------|-------|
| `language` | Yes | All supported languages |
| `domain` | Yes | Client can specify a domain (e.g. `medical`) to pass through to the transcription engine |
| `audio_format` | Yes | Client declares encoding and sample rate |
| `enable_partials` | Yes | Suppresses partials in output but still used internally |
| `enable_diarization` | Yes | Enabled by default in all profiles |
| `speaker_sensitivity` | Yes | Diarization sensitivity |
| `max_speakers` | Yes | Limit speaker count |
| `additional_vocab` | Yes | Custom vocabulary entries |
| `volume_threshold` | Yes | Audio filtering |
| `message_control` | Yes | Adjust which messages are forwarded |
| `operating_point` | No | Fixed per profile |
| `max_delay` | No | Fixed per profile |
| `end_of_utterance_mode` | No | Fixed per profile |
| `end_of_utterance_silence_trigger` | No | Fixed per profile |
| `streaming_mode` | No | Always enabled |
| `conversation_config` | No | Managed by profile/Voice SDK |
| `audio_filtering_config` | No | Managed by profile |
| `transcript_filtering_config` | No | Managed by profile |
| `speaker_diarization_config` | No | Managed by Voice SDK |
