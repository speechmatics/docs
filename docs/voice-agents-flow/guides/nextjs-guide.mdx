...
<Box asChild maxWidth="300px">
  <Card asChild>
  <a href="https://github.com/speechmatics/nextjs-flow-guide" target="_blank">
    <Flex maxWidth="300px" align="center" gap="2">
      <GithubIcon size={16} />
      <span>See complete code on GitHub</span>
    </Flex>
  </a>
  </Card>
</Box>
...
# Lets is request JWTs to authenticate with the API
...
<Tabs>
  ...
  <TabItem value="useAudioContexts.ts">
...
<Tabs>
  ...
  <TabItem value="MicrophoneSelect.tsx">
    Helper component to render the microphone select dropdown. It also handles prompting the user for permission to use the microphone.
    <CodeBlock title="components/MicrophoneSelect.tsx" language="tsx" showLineNumbers>
      {microphoneSelectExample}
    </CodeBlock>
  </TabItem>
  <TabItem value="actions.ts">
    Here we define the server action to request a JWT from the backend.
