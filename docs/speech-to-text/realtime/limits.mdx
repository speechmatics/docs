...
If a session's custom dictionary (`additional_vocab`) contains more than 20,000 items, the Realtime SaaS service sends an in-band error message of type `protocol_error` and closes the WebSocket connection with code `1003`. See [Custom dictionary content limits](/speech-to-text/features/custom-dictionary#maximum-number-of-custom-dictionary-items) for details.
