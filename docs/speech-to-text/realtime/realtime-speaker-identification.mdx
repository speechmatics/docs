---
sidebar_label: 'Realtime speaker identification'
...
To generate identifiers for a desired speaker, run a [diarization-enabled](../features/diarization.mdx) transcription on an audio sample where the speaker is ideally speaking alone.
You then request the identifiers back from the engine by sending a `GetSpeakers` request.
...
- final: false (default) — returns identifiers generated up to the point of the request. To avoid empty results, wait until the server has issued at least one `TranscriptAdded` message before sending the request.
...
Once you have speaker identifiers, you can provide them in the transcription config to identify and tag known speakers.
