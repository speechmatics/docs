...
<Card mb="4">
  ...
  <DataList.Item>
    ...
    <DataList.Value>
      <Text asChild mb="0">
        Each speaker is identified by their voice. <br/>
  ...
  <DataList.Item>
    ...
    <DataList.Value>
      <Text asChild mb="0">
        Each audio channel is transcribed separately. <br/>
        Useful when speakers are recorded on separate audio channels.
      </Text>
    </DataList.Value>
  </DataList.Item>
    <DataList.Item>
    <DataList.Label>
    [Channel & speaker diarization](#channel-and-speaker-diarization)
    </DataList.Label>
    <DataList.Value>
      <Text asChild mb="0">
        A combination of speaker and channel diarization. <br/>
        Each audio channel is transcribed separately and identifies unique speakers per channel.<br/>
        Useful when multiple speakers exist on multiple audio channels.
      </Text>
    </DataList.Value>
  </DataList.Item>
</DataList.Root>
...
Channel and speaker diarization acts combines both Speaker diarization and Channel diarization, splitting transcripts per channel whilst also separating individual speakers in each channel.
...
To send audio to a specific channel, follow the directions in [Send audio to a channel](#send-audio-to-a-channel).
...
## Configuring diarization

You can customize diarization to match your use case by adjusting settings for sensitivity, limiting the maximum number of speakers, preferring the current speaker to reduce false switches, and controlling how punctuation influences accuracy. Timeout rules also apply to ensure diarization completes reliably on long audio files.
...
- When running realtime `channel` and `channel_and_speaker` diarization, the processing of multiple audio streams can be computationally demanding and increase latency. To offset this, we highly reccomend [using a GPU](../../deployments/virtual-appliance/administration/using-a-gpu.mdx)