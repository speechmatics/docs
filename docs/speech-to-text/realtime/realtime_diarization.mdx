...
<Card mb="4">
  ...
  <DataList.Item>
    ...
    <DataList.Value>
      <Text asChild mb="0">
        Speakers identified by voice. <br/>
...
Channel and speaker diarization acts combines both [Speaker diarization](#speaker-diarization) and [Channel diarization](#channel-diarization), splitting transcripts per channel whilst also separating individual speakers in each channel.

To enable this mode, follow the steps in [enabling channel diarization](#enabling-channel-diarization) and set the `diarization` mode to `channel_and_speaker`.
