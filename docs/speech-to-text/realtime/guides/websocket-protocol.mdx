...
:::info
**Backpressure:** If you send audio faster than real-time, monitor the `AudioAdded` acknowledgements. Avoid having more than a few hundred unacknowledged chunks in flight. If the server's buffer fills, the connection may close.
:::
