...
| `{"message": "ResumeRecognition", "transcribe_accumulated_buffer": true}` | `{"message": "RecognitionResumed"}` |
...
`transcribe_accumulated_buffer` is optional on `ResumeRecognition` and defaults to `false`: audio sent while parked is acknowledged but discarded rather than transcribed on resume. Set it to `true` to transcribe that audio instead. It only has an effect when the resume preserves the timeline - a resume that can't (see above) discards the buffer either way.

## Other messages while parked

- **`EndOfStream`** ends a parked session immediately: Speechmatics replies with `EndOfTranscript` without resuming the session, and any audio buffered since the pause is discarded rather than transcribed. For a multichannel session, the same applies to `EndOfChannel`, per channel - the session only ends once every channel has been closed this way.
- **`GetSpeakers`** and **`ForceEndOfUtterance`** need a live transcriber to answer, so sending either one resumes the session first - with the normal resume delay - before replying.
- **Exception:** if you send `ForceEndOfUtterance` while parked and almost no audio has arrived since the pause, Speechmatics replies immediately with an empty final transcript and `EndOfUtterance` instead of resuming the session for essentially nothing.
