...
To improve user experience in responsive real-time scenarios it important to know when a person has finished speaking. This is especially important for voice AI, translation, and dictation use cases. Detecting an end of utterance (also known as 'end of turn') can be used to trigger actions such as generating a response in a Voice AI agent.
...
Forcing an end of utterance allows you to manually trigger an end-of-utterance detection at any point in the conversation. This is done by sending a `ForceEndOfUtterance` message:
...
If you have your own end of turn detection model, you can integrate it with our ASR by sending the `ForceEndOfUtterance` message when your model detects an end of turn.
This will trigger an end of utterance detection and send a `Final` transcript message, followed by an extra `EndOfUtterance` message.
