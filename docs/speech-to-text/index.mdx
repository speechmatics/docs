...
- [Agent STT](#agent-stt): Stream audio from a conversation and receive speaker-labelled, turn-based transcription for a voice agent
...
<Grid columns={{initial: "1", md: "3"}} gap="3" mb="5">
  ...
  <LinkCard
    icon={<MessagesSquare/>}
    direction="column"
    title="Transcribe for voice agents"
    description="Stream audio and receive speaker-labelled, turn-based transcription ready to pass to an LLM"
...
[Agent STT](/speech-to-text/agent-stt) groups the transcript into [segments](/speech-to-text/agent-stt/segmentation): punctuated, attributed to a speaker, and ready to pass to an LLM. It also marks where each speaker's [turn](/speech-to-text/agent-stt/turn-detection) starts and ends, so your agent knows when to respond.

:::info
- Build a voice agent on LiveKit, Pipecat or Vapi
