...
:::info
From **November 3rd, 2025** onwards, we are introducing limits to the length of Custom Dictionaries entries in our Realtime SaaS service. Please review the documentation below and update your integration if necessary.
:::

Any individual Custom Dictionary (`additional_vocab`) element longer than 6 words will be automatically dropped from the config before transcription starts. This applies to both the `content` and `sounds_like` fields.
