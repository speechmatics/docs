...
**Available in:** Batch, Realtime, Agent STT.
...
- We recommend specifying no more than 1000 words or phrases (per job) in your custom dictionary for best performance. SaaS on Cloud rejects jobs whose custom dictionary exceeds 20,000 items — see [Maximum number of custom dictionary items](#maximum-number-of-custom-dictionary-items)
...
**Realtime API**<br/>
  The server will send an in-band warning message of type `validation_warning` to the client prior to the `RecognitionStarted` message. See the [Realtime API Reference](/api-ref/realtime-transcription-websocket#warning) for more details.
...
## Maximum number of custom dictionary items

SaaS on Cloud rejects a job if its custom dictionary (`additional_vocab`) contains more than 20,000 items.

**Batch API**<br/>
The request is rejected with an `HTTP 400 Bad Request` response:
```json
{
  "code": 400,
  "detail": "Job config JSON is invalid. Error: additional_vocab contains 20001 items which exceeds the maximum allowed (20000)",
  "error": "Job rejected"
}
```

**Realtime API**<br/>
The server sends an in-band error message of type [`protocol_error`](/api-ref/realtime-transcription-websocket#error) and closes the WebSocket connection with code `1003`:
```text
additional_vocab has more items (20001) than allowed (20000)
```
