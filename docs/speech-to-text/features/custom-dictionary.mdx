...
The Custom dictionary feature allows a list of custom words to be added for each transcription job. This helps when a specific word is not recognized during transcription. It could be that it's not in the vocabulary for that language, for example a company or person's name. Adding custom words can improve the likelihood they will be output.
...
The custom dictionary is available with the Standard and Enhanced models for pre-recorded and streaming transcription, and with Linden 1 for agent STT. See [Feature availability](/speech-to-text/availability/).

## Custom dictionary caching
The Speechmatics Realtime SaaS caches custom dictionaries to reduce session initialization times. 
