...
- We recommend specifying no more than 1000 words or phrases (per job) in your custom dictionary for best performance. SaaS on Cloud rejects jobs whose custom dictionary exceeds 20,000 items — see [Custom dictionary content limits](#custom-dictionary-content-limits)
...
**Batch API**<br/>
The request is rejected with `HTTP 400 Bad Request` and error type `job_rejected`:
```
Job config JSON is invalid. Error: additional_vocab contains 20001 items which exceeds the maximum allowed (20000)
```

**Realtime API**<br/>
The server sends an in-band error message of type `protocol_error` and closes the WebSocket connection with code `1003`:
```
