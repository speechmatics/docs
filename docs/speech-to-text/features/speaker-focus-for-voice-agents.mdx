...
Keep the agent's own voice out of the loop. Either use the `__ASSISTANT__` double-underscore convention or add the agent's speaker to the ignore list. Otherwise the agent's audio output is transcribed straight back in and it starts talking to itself.
