...
- `UU` - Diarization is disabled or individual speakers cannot be identified (which only applies when running batch mode on CPU operating points). `UU` can appear for example if some background noise is transcribed as speech, but the diarization system does not recognise it as a speaker.
...
:::note
This feature is only available for [Batch Transcription](/speech-to-text/#batch-processing).
:::
