...
To enable the feature, set the `diarization` property to `channel` or `channel_and_speaker`, and define each channel name under `channel_diarization_labels`.
...
The Transcriber confirms by including the channel diarization labels in the `channels` field of the `RecognitionStarted` message:
...
The Transcriber responds with a `ChannelAudioAdded` message:
...
When transcripts are returned, the `channel` property identifies the channel.
For `channel_and_speaker` diarization, the `speaker` property identifies speakers:
