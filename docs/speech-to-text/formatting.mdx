...
### Output locale for multilingual transcription

[Melia 1](/speech-to-text/models#melia-1) can transcribe more than one language in the same job, so `output_locale` also accepts an array of locales, one per language present in the audio:
...
:::info
`enable_entities` is not supported on Agent STT.
:::
...
:::info
`punctuation_overrides` is not supported on Agent STT.
:::
