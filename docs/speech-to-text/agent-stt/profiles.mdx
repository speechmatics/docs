...
Streaming transcription has its own turn detection, configured with a silence threshold rather than a profile. The two are separate mechanisms: see [Turn detection](/speech-to-text/features/turn-detection) for the streaming equivalent.
