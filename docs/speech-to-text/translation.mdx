...
When using [Language Identification](/speech-to-text/batch/language-identification), if one or more of the target languages are not [supported](./translation.mdx#supported-translation-pairs) for the identified transcription language, the job will be accepted and an error message will be included in the JSON output. No translations will be returned for that language pair. 
