...
Translation can be enabled when transcribing either a **file** or in **real-time**, by using the Speechmatics SaaS, or by deploying it [On-Prem](/deployments/container/translation).

If you're new to Speechmatics, please see our guide on [Transcribing a File](/speech-to-text/batch-transcription/quickstart) or [Transcribing in Real-Time](/speech-to-text/real-time-transcription/quickstart). Once you are set up, include the following config to enable Translation:
...
Python client example to translate a file in real-time, see [here](/speech-to-text/real-time-transcription/quickstart) for more examples of Real-Time Transcription
...
- `language`: Real-time only. The translated language [ISO Language Code](https://docs.speechmatics.com/introduction/supported-languages#languages)
...
- `speaker`: The speaker label when `diarization:speaker` is set - see [here](https://docs.speechmatics.com/features/diarization/speaker-diarization#overview) about Speaker Diarization
  - Speaker labels are only available on Final translations, not Partial translations
- `channel`: The channel label when `diarization:channel` is set - see [here](https://docs.speechmatics.com/features/diarization/channel-diarization) about Channel Diarization
...
- **Punctuation:** Punctuation plays a significant role in the accuracy of translation. It is therefore recommended to avoid disabling any [Punctuation Marks](https://docs.speechmatics.com/features/punctuation-settings) or reducing the [Punctuation Sensitivity](https://docs.speechmatics.com/features/punctuation-settings) to ensure the best possible results
- **Formatting:** The translation is applied to the [written form](https://docs.speechmatics.com/features/entities#overview) transcript
...
If one or more of the target languages are not [supported](./translation.mdx#supported-translation-pairs) for the transcription language, the job will be rejected with a HTTP 400 error response.

This behaviour is different when the [transcription language is not known](./translation.mdx#unsupported-translation-pair) ahead of time.
...
When using [Language Identification](/speech-to-text/batch-transcription/language-identification), if one or more of the target languages are not [supported](./translation.mdx#supported-translation-pairs) for the identified transcription language, the job will be accepted and an error message will be included in the JSON output. No translations will be returned for that language pair. 
...
If one or more of the target languages are not [supported](./translation.mdx#supported-translation-pairs) for the source language, a warning is raised but transcription will continue. No translations will be returned for that language.
...
If you want to report an issue or get Support more urgently [Raise an Issue](https://docs.speechmatics.com/introduction/troubleshooting) instead.
