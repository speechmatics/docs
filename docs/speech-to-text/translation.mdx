...
- `speaker`: The speaker label when `diarization:speaker` is set - see [here](/features/diarization/speaker-diarization#overview) about Speaker Diarization
  - Speaker labels are only available on Final translations, not Partial translations
- `channel`: The channel label when `diarization:channel` is set - see [here](/features/diarization/channel-diarization) about Channel Diarization
...
- **Punctuation:** Punctuation plays a significant role in the accuracy of translation. It is therefore recommended to avoid disabling any [Punctuation Marks](/features/punctuation-settings) or reducing the [Punctuation Sensitivity](/features/punctuation-settings) to ensure the best possible results
- **Formatting:** The translation is applied to the [written form](/features/entities#overview) transcript
...
If you want to report an issue or get Support more urgently [Raise an Issue](/introduction/troubleshooting) instead.
