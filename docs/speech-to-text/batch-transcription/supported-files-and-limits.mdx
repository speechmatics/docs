...
* For limits on Real-time transcription, see the [Real-time Transcription supported formats and limits](/speech-to-text/real-time-transcription/supported-formats-and-limits).
...
- 50 job status requests per second (`GET` API calls). Note that Speechmatics recommends using [Notifications](/speech-to-text/batch-transcription/notifications) for job status updates in production
