...
Melia 1 supports [output locale](/speech-to-text/formatting#output-locale) in the same way as Enhanced and Standard. Because Melia 1 can transcribe more than one language in the same job, `output_locale` also accepts an array of locales, one per language present in the audio:
...
```json
{
  ...
  "transcription_config": {
    ...
    "language": "multi",
    // highlight-start
    "output_locale": ["en-GB", "cmn-Hans"]
    // highlight-end
  }
}
```

Each locale in the array must belong to a different language. Passing two locales for the same language, for example `en-GB` and `en-US` together, is rejected.
...
Linden 1 is the model powering [Agent STT](/speech-to-text/agent-stt). It returns transcription as speaker-attributed segments, with turn messages, rather than a running word stream, so the output is ready to pass to an LLM.
