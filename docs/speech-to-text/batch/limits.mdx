...
If a job's custom dictionary (`additional_vocab`) contains more than 20,000 items, the job is rejected with `HTTP 400 Bad Request` and error type `job_rejected`. See [Custom dictionary content limits](/speech-to-text/features/custom-dictionary#maximum-number-of-custom-dictionary-items) for details.
