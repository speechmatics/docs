...
3. Configure a notification callback. You can see how to do that [here](/speech-to-text/batch/notifications).

If you are getting started with the API, please refer to the guide on how to [Transcribe a File](/speech-to-text/batch/quickstart).

## Check single job status

If you wish to retrieve a particular job, you can do so using the job ID for up to 7 days, after which time it will be automatically deleted in accordance with our [Data Retention Policy](/speech-to-text/batch/supported-files-and-limits#data-retention-limits).
