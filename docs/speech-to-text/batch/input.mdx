...
The Melia 1 model transcribes audio containing more than one language. Select it with `"model": "melia-1"` and `"language": "multi"`; for basic model selection, refer to [Models](/speech-to-text/models).

If you know which languages appear in the audio, provide them as language hints to reduce the chance of unexpected languages or scripts in the output. This config hints that the audio contains English and Arabic:
...
Specify any number of [supported language codes](/speech-to-text/languages#transcription-languages). For a monolingual file, hinting the single language present can improve accuracy.
