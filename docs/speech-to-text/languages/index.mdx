...
Set the `language` property to a language code. To detect the language automatically instead of setting it, use [language identification](/speech-to-text/batch/language-identification).
...
Melia 1 transcribes audio containing more than one language, including speakers who switch language mid-conversation, and returns a single continuous transcript. It requires `"language": "multi"` and has no language pack selection. You can list the languages you expect as [language hints](/speech-to-text/batch/input#language-hints) to improve accuracy, which does not restrict the model to those languages.
